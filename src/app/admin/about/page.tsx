...
import { DEFAULT_ABOUT } from "@/lib/pagedata";
...
export default function AdminAboutPage() {
  const [sections, setSections] = useState<ContentSection[]>(DEFAULT_ABOUT);
  const [saving, setSaving] = useState(false);
  const [saved, setSaved] = useState(false);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch("/api/admin/page-content?slug=about")
      .then((r) => r.json())
      .then(({ sections: data }) => {
  ...
  async function handleSave() {
    ...
    try {
      const res = await fetch("/api/admin/page-content", {
        ...
        body: JSON.stringify({ slug: "about", label: "About", sections }),
      });
  ...
  return (
    <div className="max-w-2xl">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl text-gray-800" style={{ fontFamily: "var(--font-display)" }}>About Page</h1>
          <p className="text-gray-400 text-xs mt-1">Edit the body sections shown on the About page</p>
        </div>
