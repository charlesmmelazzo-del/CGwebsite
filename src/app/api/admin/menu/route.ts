...
export async function GET() {
  try {
    ...
    const [tabsRes, itemsRes] = await Promise.all([
      sb.from("menu_tabs").select("*").order("order"),
      sb.from("menu_items").select("*").order("order"),
    ]);
    ...
    console.error("[GET /api/admin/menu]", e);
    return NextResponse.json({ tabs: [], items: [] });
  }
}

// POST — full replace: upsert everything in the payload, delete anything not in it
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: existingTabs } = await sb.from("menu_tabs").select("id");
    const tabsToDelete = (existingTabs ?? [])
      .map((r) => r.id)
      .filter((id) => !newTabIds.includes(id));
    if (tabsToDelete.length) {
      // Cascade: also remove their items first (in case FK doesn't cascade)
      await sb.from("menu_items").delete().in("tab_id", tabsToDelete);
      await sb.from("menu_tabs").delete().in("id", tabsToDelete);
    }

    // 2. Find and delete items that were removed (within remaining tabs)
    const { data: existingItems } = await sb.from("menu_items").select("id");
    const itemsToDelete = (existingItems ?? [])
      .map((r) => r.id)
      .filter((id) => !newItemIds.includes(id));
    if (itemsToDelete.length) {
      await sb.from("menu_items").delete().in("id", itemsToDelete);
    }
    ...
    if (newItemIds.length) {
      const { error: itemErr } = await sb.from("menu_items").upsert(
        items.map((i: {
...
// DELETE — remove a tab or item by id
export async function DELETE(req: NextRequest) {
  try {
    ...
    const sb = getSupabaseAdmin();
    if (type === "tab") {
      // Delete the tab and all its items
      await Promise.all([
        sb.from("menu_items").delete().eq("tab_id", id),
        sb.from("menu_tabs").delete().eq("id", id),
      ]);
    } else {
