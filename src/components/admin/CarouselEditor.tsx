...
import { Plus, Trash2, GripVertical, AlignLeft, AlignCenter, AlignRight, RefreshCw, Loader2, AlertTriangle } from "lucide-react";
...
export type ItemType = "text" | "image" | "form" | "instagram";
...
const ADD_TYPE_LABELS: Record<ItemType, string> = {
  text: "Text Slide",
  image: "Image Slide",
  form: "Form Slide",
  ...
  return (
    ...
    >
      ...
      <div className="p-4">
        ...
        {/* ── Schedule (all types) ── */}
        {enableScheduling && <ScheduleSection item={item} onUpdate={(p) => onUpdateItem(item.id, p)} />}
      </div>
    </div>
  );
}
...
// ─── Schedule section ─────────────────────────────────────────────────────────
function scheduleStatus(startDate?: string, endDate?: string): { text: string; cls: string } {
  const now = new Date();
  if (!startDate && !endDate) return { text: "Always visible (when active)", cls: "text-gray-400" };
  const start = startDate ? new Date(startDate) : null;
  const end   = endDate   ? new Date(endDate)   : null;
  if (end && end < now)    return { text: `Expired — ended ${end.toLocaleDateString()}`, cls: "text-amber-500" };
  if (start && start > now) return { text: `Scheduled — starts ${start.toLocaleDateString()}`, cls: "text-blue-500" };
  if (end) return { text: `Live now — ends ${end.toLocaleDateString()}`, cls: "text-green-600" };
  return { text: "Live now (no end date)", cls: "text-green-600" };
}
...
function ScheduleSection({
  ...
  item: CarouselItem;
  onUpdate: (p: Partial<CarouselItem>) => void;
}) {
  const status = scheduleStatus(item.startDate, item.endDate);
  return (
    <div className="border-t border-gray-100 pt-3 mt-3 space-y-2">
      ...
      <div className="grid grid-cols-2 gap-2">
        <div>
          ...
          <input
            type="datetime-local"
            value={item.startDate ? item.startDate.slice(0, 16) : ""}
        ...
        <div>
          ...
          <input
            type="datetime-local"
            value={item.endDate ? item.endDate.slice(0, 16) : ""}
            onChange={(e) => onUpdate({ endDate: e.target.value || undefined })}
            className={inputCls}
          />
        </div>
      </div>
      <p className={`text-[10px] ${status.cls}`}>{status.text}</p>
    </div>
  );
}
...
// ─── CarouselEditor (controlled) ──────────────────────────────────────────────
export default function CarouselEditor({
  items,
  onChange,
  allowedTypes = ["text", "image", "form", "instagram"],
  ...
  const [addType, setAddType] = useState<ItemType>(allowedTypes[0] ?? "text");
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    onChange(arrayMove(items, oldIndex, newIndex).map((item, idx) => ({ ...item, order: idx })));
  }
  ...
  function updateItem(id: string, patch: Partial<CarouselItem>) {
    onChange(items.map((i) => i.id === id ? { ...i, ...patch } as CarouselItem : i));
  }
    ...
    <>
      ...
      <div className="flex items-center gap-2 p-3 border border-dashed border-gray-200 rounded-sm">
        <select value={addType} onChange={(e) => setAddType(e.target.value as ItemType)} className="bg-white border border-gray-200 text-gray-500 text-xs px-2 py-1.5 outline-none rounded-sm">
          {allowedTypes.map((t) => (
            <option key={t} value={t}>{ADD_TYPE_LABELS[t]}</option>
          ))}
        </select>
